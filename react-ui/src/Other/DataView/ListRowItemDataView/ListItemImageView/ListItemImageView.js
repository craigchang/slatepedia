...
const ListItemImageView = ({imageFolder, name, fileType, cssClassName, spriteSheet}) => {
  ...
  if ((spriteSheet === 'weapons' || spriteSheet === 'bows' || spriteSheet === 'shields' || spriteSheet === 'monsters' || spriteSheet === 'food' || (spriteSheet === 'materials' && cssClassName))) {
    const spriteClassName = (cssClassName || name.replace(/ /g, '-').replace(/'/g, '').toLowerCase()).replace(/'/g, '');
    const baseClass = spriteSheet === 'weapons' ? 'weapon-sprite weapon-sprite-sm' : spriteSheet === 'bows' ? 'bow-sprite bow-sprite-sm' : spriteSheet === 'shields' ? 'shield-sprite shield-sprite-sm' : spriteSheet === 'monsters' ? 'monster-sprite monster-sprite-sm' : spriteSheet === 'food' ? 'food-sprite food-sprite-sm' : 'material-sprite material-sprite-sm';
    return (
